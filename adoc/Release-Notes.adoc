...
For information about deploying {product}, see the _Deployment Guide_ at
{deployment-url}.
...
----
+
Replace `<PASSWORD>` with the password from step 3. The URL is a modified version of URL in step 2: as the subdomain name, use `cf-usb-cf-usb` instead of `cf-usb`.
