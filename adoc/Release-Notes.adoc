...
[IMPORTANT] 
====
* With {product} 1.5.2, the database seeder, UAA and credhub will use TLS by default to connect with an external database. Other SCF components will still use the unencrypted TCP protocol. If you would like to configure the seeder, UAA & Credhub services to connect via insecure means in a trusted network, you will need to use the following parameters:
...
* {product} 1.5.2 has been tested successfully with external database instances hosted on Amazon RDS MariaDB and Azure MariaDB. Amazon Aurora, Amazon RDS MySQL or Azure MYSQL are not yet supported configurations.  

* `cf-usb` will not become ready when {product} 1.5.2 is deployed with an external database. It is recommended to use `minibroker` as a service broker when {product} 1.5.2 is deployed with an external database server.
...
* The `cf-usb` service broker will not work on {aks} deployments due to a hard-coded username. We recommend using the service brokers that are provided by {azure} for OSBAPI connections to their hosted database services. 
