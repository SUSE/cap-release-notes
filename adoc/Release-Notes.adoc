...
1. Install 1.5, HA but scale mysql to 1 
UAA:
helm install --name scf-uaa --namespace uaa suse/uaa -f <values.yaml> --set config.HA=true

SCF:
helm install --name scf-cf --namespace scf suse/cf -f <values.yaml> --set config.HA=true --set "secrets.UAA_CA_CERT=${CA_CERT}"

Steps to upgrade: [verified by Docs, need to format properly for AsciiDocs]

1. Upgrade from 1.4.1, HA but scale mysql to 1 
UAA:
...
2. Upgrade to 1.5RC, HA but mysql at 1 
UAA:
