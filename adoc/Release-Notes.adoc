...
* Support for Amazon AWS service brokers 
* Credhub now available for operators (disabled by default)
* App-autoscaler now supported for :azure: AKS & Amazon EKS
* Minibroker now supported for Redis, MongoDB, MySQL, Postgres & MariaDB
* Stratos UI is now v2.2 with these new features:
** 
...
* :azure: instances should now be increased to `Standard_D4_v2` or larger from the previously documented `Standard_D2_v2` due to the new features.

* Credhub on :azure: is considered experimental.

* Minibroker with MariaDB will see timeout issues upon deletion, so if an error appears, then wait a minute and retry.

* As of writing, the Amazon AWS service broker has become v1.0 and some changes have crept in since the beta version that will require an update to the associated CAP Helm chart. THIS MAY LIKELY BE REMOVED BUT FOR CONTEXT PER REVIEW

* Universal service brokers will require a workaround post-upgrade to reconnect with SCF/CAP as the URL has changed:

** Run `kubectl get secret --namespace scf` and copy the name of the secret, which will have the syntax `secrets-2.14.5-1`
** Run `cf service-brokers` to get the URL for the `cf-usb` host, which will look similar to `https://cf-usb.scf.svc.cluster.local:24054`
...
** Finally, update the service broker via `cf update-service-broker usb broker-admin <password> https://cf-usb-cf-usb.scf.svc.cluster.local:24054`, where <password> is the password from the third step, and the URL is the result of the second step with the leading `cf-usb` part doubled up with a dash as a separator.
