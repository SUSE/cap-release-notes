...
** Minibroker support for Redis, MongoDB, {mysql}, {postgre}, & MariaDB
...
* Enabling new feature roles such as Credhub and App-AutoScaler will require more memory and CPU resources in minimal installations (at least 22 GB in total for single instances of all roles enabled). For example, {azure} instances should now be increased to `Standard_D4_v2` or larger from the previously documented `Standard_D2_v2` if the new feature pods are enabled.
...
* The URL of the internal cf-usb broker endpoint has changed. Brokers for {postgre} and {mysql} that use cf-usb will require the following manual fix after upgrade to reconnect with SCF/CAP:
