...
[arabic]
.. Install HA UAA but scale the `mysql` role to 1 as a transition step. (For these commands, we will assume that `scf-uaa` and `scf-cf` are the chart names.)
+
[source,bash]
----
...
* If you are using the `uaa` embedded in the `suse/cf` chart, note that automatic ingress creation via helm is not working with that role. Therefore, point the Ingress controller to the `uaa` service created via the `cf` Helm chart manually. Alternatively, deploy `uaa` separately via the `suse/uaa` chart, which will create Ingress pointer automatically.

[id='sec.1_5.feature']
==== Features and Fixes
* `cf-deployment` has been updated to version 9.5.0.
* Eirini updated to 0.0.14
* PXC (Percona XtraDB Cluster) replaces cf-mysql for database management -- PLEASE READ IMPORTANT SECTIONS ABOVE
