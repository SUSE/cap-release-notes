...
* When updating or scaling down instances of an app, there may be older instances of the app that remain running and accepting traffic in what Diego deems as "long-running processes" (or LRPs). If seen, then there may be an instance of the `cc-clock` role that did not come up properly due to an incorrect protocol. To address this:
