...
** Added support for {ng} Ingress Controller with customizable Ingress via user suppplied annotations.
...
* Fixed the test for an insecure docker registry (use tcpdomain for the route)
...
* If you are using the {ng} Ingress Controller and seeing `Request Entity Too Large` errors you should bump up the ingress proxy body size to an appropriate value by setting the `ingress.annotations` key in helm chart values as in the following:
