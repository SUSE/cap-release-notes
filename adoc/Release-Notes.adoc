...
[id='sec.1_5_1.new']
==== What Is New?
* {scf} has been updated to version 2.19.0:
** Support for Eirini SSH feature
** Support for external CF-related databases, including CCDB & UAA when embedded in SCF
** Support for Eirini to run on GKE, EKS and CRI-O based Kubernetes environments
...
* On occasion, the `uaa` pod may fail to start due to database migration failures. In these situations, manual intervention is required to track the last completed transaction in `uaadb` database, update the flyway schema history table with the record of the last completed transaction, then restart the migration. Please contact support for further instructions.
