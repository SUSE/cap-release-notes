...
IMPORTANT: In order to deploy to this version, or upgrade from {scf} 2.17.1, part of {cap} 1.4.1, to this version, you will need to scale down the database roles to 1 pod in order to migrate from cf-mysql to PXC. This is based on upstream instructions but based on what we've seen with other components that rely on the database, such as Credhub, scaling all database roles into single availability helps with a stable migration and deployment

Steps to deploy: [verified by Docs, need to format properly for AsciiDocs]

1. Install 1.5, HA but scale the `mysql` role to 1 as a transition step. 
...
3. Scale the `mysql` roles up to the default HA configuration.
