...
* Support for Amazon AWS Service Broker 
* Credhub now available to CF apps and compatible brokers (disabled by default)
* App-AutoScaler now supported for :azure: AKS & Amazon EKS (disabled by default)
* Minibroker support for Redis, MongoDB, MySQL, Postgres, & MariaDB
* Stratos UI is now v2.2 including:
** New Metrics endpoint for keeping and exposing CF application and Kubernetes metrics
** New views for Kubernetes application, pod, and node metrics
** Many more new features and fixes. See https://github.com/SUSE/stratos/releases/tag/2.2.0
...
* Enabling new feature roles such as Credhub and App-AutoScaler will require more memory and CPU resources in minimal installations(at least 22GB in total for single instances of all roles enabled). For example, :azure: instances should now be increased to `Standard_D4_v2` or larger from the previously documented `Standard_D2_v2` if the new feature pods are enabled.

* Credhub on :azure: is considered experimental.

* Minibroker with MariaDB will see timeout issues upon deletion. If an error appears, wait one minute and retry. If `cf delete-service ...` fails but the instance pods are removed from Kubernetes, the service instance data can be safely removed with `cf purge-service-instance ...`.

* Amazon AWS Service Broker has changed with the recent release of v1.0. The Helm chart from SUSE will be updated to include these changes in the near future. 

* The URL of the internal cf-usb broker endpoint has changed. Brokers for PostgreSQL and MySQL that use cf-usb will require the following manual fix after upgrade to reconnect with SCF/CAP:

** Run `kubectl get secret --namespace scf` and copy the name of the secret (e.g. `secrets-2.14.5-1`)
** Run `cf service-brokers` to get the URL for the `cf-usb` host (e.g. `https://cf-usb.scf.svc.cluster.local:24054`)
** Get the current CF_USB password by running `kubectl get secret --namespace scf <secret_name> -o yaml | grep \\scf-usb-password: | cut -d: -f2 | base64 -id`, where <secret_name> is the name from the first step
** Finally, update the service broker via `cf update-service-broker usb broker-admin <password> https://cf-usb-cf-usb.scf.svc.cluster.local:24054`, where <password> is the password from the third step, and the URL is the result of the second step with the leading `cf-usb` part doubled with a dash separator.
...
* Starting with {product} 1.2.1, during helm upgrade, Kubernetes will not upgrade pods that are not ready by default. In order to upgrade all pods, use the complete command here: `helm upgrade --force --recreate-pods`

* Similar to {caaspa} 3, {azure} now mandates a stricter security policy via PodSecurityPolicy (PSP), which is included as part the {product} Deployment Guide. Any namespace tied to {product} requires privileged ports to be accessible needs to have to have a PSP set appropriately for access. This would include the default conventions of `scf`, `uaa`, `stratos-ui`, `mysql-sidecar` and `postgres-sidecar` as per our documentation tied to {caaspa} 3: https://www.suse.com/documentation/cloud-application-platform-1/book_cap_deployment/data/sec_cap_prereqs-prod.html#sec_cap_caasp-3

* {azure} users who previously had a Kubernetes policy without RBAC, but now have Microsoft Azure Kubernetes Service (AKS) with RBAC (which is the new default with AKS), will need to modify their `scf-config-values.yaml` files so that `auth: rbac` replaces `auth: none`. If you remain in an AKS policy without RBAC, then you can ignore this change.

* If you are using {azure}, ensure that the root partition has enough space for the installation and potential upgrades. To do so, add the parameter `--node-osdisk-size=60` to the command that creates the AKS (Azure Kubernetes Service) instance: `az aks create`. For the complete command, the {product} _Deployment Guide_ has a section under the AKS section called `Create Resource Group and AKS Instance`: https://www.suse.com/documentation/cloud-application-platform-1/book_cap_deployment/data/sec_cap_create-aks-instance.html
