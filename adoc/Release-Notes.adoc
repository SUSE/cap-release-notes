...
[arabic]
.. Install 1.5 HA UAA but scale the `mysql` role to 1 as a transition step. (For these commands, we will assume that `scf-uaa` and `scf-cf` are the chart names.)
+
[source,bash]
----
...
[arabic]
.. Before upgrading from 1.4.1, while keeping HA, scale the `mysql` role to 1. (Note: the subsequent instructions assume that the charts were installed with `config.HA` set to `true`.)
+
[source,bash]
----
