...
* Resuming a past practice, with {product} 1.4, during helm upgrade, {k8s} will not upgrade pods that are not ready by default, particularly mysql-related pods. To upgrade all pods, use the complete command: `helm upgrade --force --recreate-pods`. This will reintroduce downtime for apps but ensures the upgrade doesn't break.
