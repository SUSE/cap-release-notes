...
* If you are using the USB sidecars with an existing {product} installation and you upgrade, you may run into issues whereby it’s impossible to delete service instances or apps. You’ll need to run the following command to update one of the passwords associated with the sidecars:
...
* If you see the following behaviour during an upgrade to {product} 1.1.1:
** The `diego-api` role is not fully functional (in other words, appearing as `(0/1)`)
** The `bbs` job in the pod which is not starting, as per `monit summary`
** The bbs stdout log `/var/vcap/sys/log/bbs/bbs.stdout.log` contains an entry that says _Error 1062: Duplicate entry 'version' for key 'PRIMARY'_
+
Then do the following to unblock the upgrade:
