...
* Enabling new feature roles (e.g. Credhub and App-AutoScaler) will require more memory and CPU resources in minimal installations. For example, :azure: instances should now be increased to `Standard_D4_v2` or larger from the previously documented `Standard_D2_v2` if the new feature pods are enabled.

* Credhub on :azure: is considered experimental.

* Minibroker with MariaDB will see timeout issues upon deletion. If an error appears, wait one minute and retry.
